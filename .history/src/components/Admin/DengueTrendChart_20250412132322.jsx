...
export default function DengueTrendChart() {
  return (
    <ChartContainer className="min-h-[260px] bg-primary w-full flex flex-col gap-2">
      ...
      {/* Custom Legend */}
      <div className="flex justify-start gap-14 ml-8">
        {severityLevels.map(({ label, color }) => (
          <div key={label} className="flex items-center gap-2">
            ...
            />
            <span className="text-lg text-base-content">{label}</span>
          </div>
        ))}
      </div>
    </ChartContainer>
