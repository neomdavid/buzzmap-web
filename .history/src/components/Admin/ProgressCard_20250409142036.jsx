...
const ProgressCard = ({
  ...
  return (
    <div className="flex flex-col gap-1.5 bg-white p-5 rounded-3xl shadow-sm">
      ...
      {/* Progress Bar */}
      <div className="w-full bg-gray-200 h-6.5 rounded-full my-2">
        <div
      ...
      <div className="flex flex-col gap-3">
        {items.map((item, index) => (
          <div className="flex justify-between" key={index}>
            ...
            {item.type === "pending" && (
              <button
                className="text-xs bg-base-content text-white font-light p-2.5 rounded-full hover:cursor-pointer hover:bg-base-content/70 transition-all duration-200"
                onClick={onEdit}
              >
