...
const ReportCard = ({
  ...
  return (
    <article className="flex flex-col z-[10000000] hover:bg-red-100">
      <div
        className={`flex flex-col shadow-sm ${topBg} text-white rounded-3xl py-6 pb-3 px-8`}
      >
        <p className="text-xl">{title}</p>
        <h1 className="text-8xl">{count}</h1>
      </div>

      <div className="flex flex-col shadow-md bg-base-200 rounded-3xl gap-y-3 px-8 py-4 pt-15 mt-[-34px] z-[-4]">
        {type === "status" &&
          items.map((item, i) => (
