  ...
  return (
    ...
    >
      <div className="modal-box bg-white rounded-3xl shadow-2xl w-9/12 max-w-4xl p-12 relative">
        <button
          className="absolute top-30 right-10 text-2xl font-semibold hover:text-gray-500 transition-color duration-200 hover:cursor-pointer"
          onClick={onClose}
        >
          ✕
        </button>
          ...
          {images?.length > 0 && (
            <div className="grid grid-cols-2 gap-4 w-full">
              {images.map((image, index) => (
                <div
                  key={index}
                  className="rounded-md overflow-hidden shadow-lg h-65" // Constrain container height
                >
                  <img
