...
const VerifyReportModal = ({
  ...
  useEffect(() => {
    if (isLoaded && coordinates?.length === 2) {
      ...
      if (streetViewRef.current && showStreetView) {
  ...
  }, [isLoaded, coordinates, showStreetView]);
        ...
        {!showConfirmation ? (
          <>
            ...
            <div className="space-y-6">
              ...
              {/* Show street view if toggled and there are photos */}
              {showStreetView &&
                images?.length > 0 &&
                coordinates?.length === 2 && (
                  <>
                    <p className="text-left text-2xl font-bold">Street View</p>
                    <hr className="text-accent/50 mb-4" />
                    <div
                      className="w-full h-64 mt-4 shadow-sm overflow-hidden rounded-2xl"
                      ref={streetViewRef}
