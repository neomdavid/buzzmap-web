...
function ReportTable2({ posts, isActionable = true }) {
  ...
  const rowData = posts.map((post) => ({
    ...
    location: post.specific_location?.coordinates
      ? `Barangay: ${
          post.barangay
        }, Coordinates: ${post.specific_location.coordinates.join(", ")}`
      : "No coordinates available", // Handle missing coordinates gracefully
    // Date formatting
    ...
    coordinates: post.specific_location?.coordinates || [], // Pass coordinates array as is
  }));
