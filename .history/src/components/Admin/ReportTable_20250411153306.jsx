...
const ReportTable = ({ rows, hasActions = false }) => {
  ...
  const actionColumn = columnHelper.display({
    ...
    header: "Actions",
    cell: () => (
      <div className="flex gap-4 text-primary text-[12px]">
        <button className="flex items-center gap-1 hover:bg-gray-100 p-3">
        ...
        <button className="flex items-center text-error gap-1">
    ...
    <table className="w-full">
      ...
      <tbody className="space-y-4">
        {table.getRowModel().rows.map((row) => (
          <tr
            key={row.id}
            className="flex flex-col justify-center text-black text-[13.5px] align-top"
          >
