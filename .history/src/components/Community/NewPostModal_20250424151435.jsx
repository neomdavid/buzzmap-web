...
export default function MapPicker({ onLocationSelect }) {
      ...
      for (let f of barangayData.features) {
        ...
        for (let polyCoords of polys) {
          ...
          if (turf.booleanPointInPolygon(pt, poly)) {
            setSelectedBarangay(f.properties.name);
  ...
  return (
    <div style={{ position: "relative" }}>
      {/* Dropdown for searching barangays */}
      <select
        onChange={(e) => {
      ...
      >
        ...
        {/* Transparent clickable rectangle */}
        <Rectangle
          bounds={QC_BOUNDS}
          options={{
            fillOpacity: 0,
        ...
        {markerPosition && <Marker position={markerPosition} label="📍" />}
      </GoogleMap>
      ...
      {markerPosition && (
        <div style={{ marginTop: "10px", fontSize: "14px" }}>
          ...
          {selectedBarangay && (
            <>
              <strong>Barangay:</strong> {selectedBarangay}
            </>
          )}
        </div>
      )}
