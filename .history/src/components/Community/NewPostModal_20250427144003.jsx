...
const NewPostModal = ({ onSubmit }) => {
  ...
  // Google Maps context (from GoogleMapsProvider)
  ...
  const geocodeCoordinates = (lat, lng) => {
    if (!isLoaded) return; // Wait until Google Maps is loaded

    const geocoder = new window.google.maps.Geocoder(); // Google Maps Geocoder

    geocoder.geocode(
      { location: { lat: parseFloat(lat), lng: parseFloat(lng) } },
      (results, status) => {
        if (status === "OK" && results[0]) {
          setGeocodedLocation(results[0].formatted_address); // Update state with the address
        } else {
