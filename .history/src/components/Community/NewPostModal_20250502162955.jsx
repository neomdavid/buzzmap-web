...
const NewPostModal = ({ onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("barangay", barangay); // Barangay
      formData.append("specific_location[type]", "Point"); // Location Type
      formData.append("specific_location[coordinates][0]", lng); // Longitude
      formData.append("specific_location[coordinates][1]", lat); // Latitude
      formData.append(
        "date_and_time",
        new Date(`${date}T${time}`).toISOString()
      ); // Date and time in ISO format
      formData.append("report_type", reportType); // Report Type
      ...
      if (images.length > 0) {
        ...
        images.forEach((img) => {
          formData.append("images", img); // Append image file(s)
        });
      }

      console.log("📦 Final FormData body:", formData);

      // Make the POST request to create the report with the FormData
      const response = await createPostWithImage(formData).unwrap(); // Assuming this sends the request to the API
      ...
      // If there's an error during submission, handle it gracefully
