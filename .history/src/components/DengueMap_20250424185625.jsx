  ...
  const handleLocationSelect = (coords) => {
    ...
    if (barangayData) {
      ...
      for (let f of barangayData.features) {
        ...
        // Debug: Check the current barangay's risk level and geometry type
        console.log(
          `Barangay: ${f.properties.name}, Risk: ${f.properties.risk}`
        );
        for (let polyCoords of polys) {
          ...
          if (turf.booleanPointInPolygon(pt, poly)) {
            console.log(`Point inside barangay: ${f.properties.name}`);
    ...
    <div className="w-full h-full">
      ...
      >
        ...
        {barangayData?.features.map((feature, index) => {
          ...
          // Debug: Check if riskColor is being applied
          console.log(
            `Rendering ${feature.properties.name} with color ${riskColor}`
          );
