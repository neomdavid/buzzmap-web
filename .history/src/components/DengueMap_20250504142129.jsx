...
const DengueMap = ({
  ...
  const handlePolygonClick = (feature) => {
    ...
    const center = turf.center(feature.geometry); // Get center of the polygon
    mapRef.current.panTo(center.geometry.coordinates); // Pan the map to that center
  ...
  return (
    <div className="w-full" style={{ height: height }}>
      ...
      >
        ...
        {/* Display InfoWindow for selected Barangay */}
        {selectedBarangay && (
          <InfoWindow
            position={{
              lat: selectedBarangay.geometry.coordinates[0][0][1],
              lng: selectedBarangay.geometry.coordinates[0][0][0],
            }}
            onCloseClick={() => setSelectedBarangay(null)} // Close the info window
          >
            <div>
              <h3>{selectedBarangay.properties.name}</h3>
              <p>
                <strong>Dengue Cases:</strong> 20+ reported in the past 2 weeks
              </p>
              <p>
                <strong>Recommendation:</strong> Immediate fogging and community
                awareness drive required.
              </p>
              <p>
                <strong>Rationale:</strong> Barangay{" "}
                {selectedBarangay.properties.name} has seen a significant rise
                in dengue cases.
              </p>
            </div>
          </InfoWindow>
