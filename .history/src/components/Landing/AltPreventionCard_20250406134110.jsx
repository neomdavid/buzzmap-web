...
const AltPreventionCard = ({
  ...
  return (
    <section
      className={`min-w-[200px] h-[290px] lg:min-w-[250px] lg:h-[340px] relative rounded-[37px] xl:h-[400px] xl:min-w-[300px]  flex ${bgColor}`}
    >
      ...
      {Icon && (
        <Icon
          size={iconSize}
          weight="fill"
          className={`absolute text-primary ${iconPosition}`}
      ...
      {/* Title */}
      <p
        className={`absolute mx-2 text-4xl leading-8 text-primary font-bold  lg:text-[32px] lg:leading-9 xl:text-[40px] xl:leading-10 ${titlePosition} text-${titleAlign}`}
      >
        {title}
      </p>
      ...
      {/* Subtext */}
      <p
        className={`absolute text-sm max-w-[140px] font-normal text-primary opacity-80 ${subtextPosition} text-${subtextAlign}`}
      >
        {subtext}
      </p>
