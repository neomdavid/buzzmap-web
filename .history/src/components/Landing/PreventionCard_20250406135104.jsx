...
const PreventionCard = ({ title, category, bgImg, to }) => {
  ...
  return (
    <section className=" relative min-w-[200px] h-[290px] lg:min-w-[250px] lg:h-[340px] xl:h-[400px] xl:min-w-[300px]">
      <div
        className="absolute inset-0 bg-cover rounded-[37px] bg-center mix-blend-luminosity"
        style={{ backgroundImage: `url(${bgImg})` }}
      ></div>

      <p className="text-3xl lg:text-4xl text-white absolute bottom-9 left-5 leading-7.5 font-bold w-[100px] lg:w-[140px] lg:leading-8.5 xl:text-[40px] xl:leading-11 xl:w-[180px]">
        {title}
      </p>
      <p className="text-xs font-normal p-2 px-4 rounded-full italic absolute bottom-41 lg:bottom-46 xl:bottom-55 xl:text-[11px]  left-5 leading-7.5 font-bold text-primary bg-white">
        {category}
      </p>
