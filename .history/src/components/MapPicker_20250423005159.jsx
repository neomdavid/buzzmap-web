...
const MapPicker = ({ onLocationSelect }) => {
  ...
  const fetchBarangay = async (coords) => {
    ...
    if (barangayComponent) {
      ...
      onLocationSelect(coords, barangayComponent.long_name); // Send both coordinates and barangay to parent
    } else {
      setBarangay("Barangay not found");
      onLocationSelect(coords, "Barangay not found"); // Send both coordinates and default message to parent
    }
  };
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      {
        enableHighAccuracy: true, // This ensures the geolocation request is more accurate
        timeout: 10000, // Timeout after 10 seconds
        maximumAge: 0, // Avoid using cached geolocation
      }
    );
  }, [qcPolygonPaths]);
