...
const MapPicker = ({ onLocationSelect }) => {
  ...
  const fetchBarangay = async (coords) => {
    ...
    // If the result is Quezon City, handle it separately
    const cityComponent = result.address_components.find((comp) =>
      comp.types.includes("locality")
    );

    if (cityComponent && cityComponent.long_name === "Quezon City") {
      setBarangay("Quezon City"); // Set the barangay to Quezon City if it's explicitly found
      return;
    }

    // Otherwise, continue checking for barangay-related components
    const barangayComponent = result.address_components.find(
      (comp) =>
        comp.types.includes("sublocality_level_2") || // barangay level
        comp.types.includes("sublocality") || // Check if it's just a sublocality
        comp.types.includes("neighborhood") || // Sometimes 'neighborhood' is used
        comp.types.includes("locality") || // Sometimes, this might indicate the barangay
        comp.types.includes("administrative_area_level_2") // Local area or district level
    );
  ...
  const handleMapClick = (e) => {
    ...
    if (isWithinQC) {
      ...
      setMarkerPosition(clickedLatLng);
      onLocationSelect(clickedLatLng, "Quezon City");
      setIsLocationPinned(true); // Mark location a
