...
const Navbar = () => {
  ...
  if (currentRoute === "/mapping") {
    ...
    return (
      <nav className="z-100000 fixed right-12 top-8 text-white text-md bg-primary py-3.5 px-6 rounded-2xl shadow-[0px_0px_10px_rgba(0,0,0,0.3)] flex justify-center gap-x-8">
        {navLinks.map((link) => (
          <NavLink
            key={link.name}
            to={link.to}
