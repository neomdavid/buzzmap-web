  ...
  const renderProfile = (darkMode = false) => (
    <>
      <div className="dropdown dropdown-end">
        ...
        <ul
          tabindex="0"
          className="dropdown-content menu bg-primary text-white rounded-xl z-1 w-120 p-6 py-8 shadow-sm flex flex-col justify-center "
        >
          <div className="w-full flex justify-center mb-3">
            <IconUserCircle size={50} className="text-white " />
          </div>
          <p className="text-center font-bold text-2xl">{user.name}</p>
          <p className="text-center font-light text-white text-lg">
            {user.email}
          </p>
          <hr className="text-gray-200 mt-4 mb-2 w-[110%]" />
        </ul>
      </div>
    </>
  );
