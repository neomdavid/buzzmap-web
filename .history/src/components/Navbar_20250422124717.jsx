    ...
    <>
      <div className="dropdown dropdown-end">
        <div
          tabIndex="0"
          role="button"
          className="flex items-center gap-2 px-3 py-2 rounded-full border border-gray-300 transition hover:shadow-sm hover:bg-gray-100/10"
        >
          <span
        ...
        <ul
          tabIndex="0"
          className="dropdown-content menu bg-primary text-white backdrop-blur-xl bg-opacity-90 rounded-2xl shadow-xl w-80 mt-2 overflow-hidden z-50"
        >
          {/* Header */}
          <div className="p-6 flex flex-col gap-1 items-center">
            <IconUserCircle size={60} className="text-white mb-2" />
            <p className="text-center font-semibold text-xl tracking-tight">
              {user.name}
            </p>
            <p className="text-center text-sm text-white/80">{user.email}</p>
          </div>
          ...
          <button
            onClick={() => {
              // TODO: Add logout dispatch here
              console.log("Logged out");
            }}
            className="w-full px-6 py-4 text-left text-white text-md hover:bg-white/10 transition-colors duration-200"
          >
            Logout
          </button>
        </ul>
