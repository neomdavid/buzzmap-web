  ...
  if (currentRoute === "/mapping") {
    return (
      <nav className={`${baseClass} bg-primary text-white`}>
        ...
        {isOpen && (
          <div className="absolute top-full left-0 w-full bg-primary px-6 py-4 flex flex-col gap-y-4 md:hidden transition-all duration-300 transform ease-in-out translate-y-0">
            {renderLinks(true)}
            {user.name !== "Guest"
              ? renderProfile(true)
              : renderLoginButton(true)}
    ...
    return (
      <nav className={`${baseClass} bg-white shadow-sm`}>
        ...
        {isOpen && (
          <div className="absolute top-full left-0 w-full bg-white px-6 py-4 flex flex-col gap-y-4 md:hidden shadow-md transition-all duration-300 transform ease-in-out translate-y-0">
            {renderLinks(true)}
            {user.name !== "Guest" ? renderProfile() : renderLoginButton()}
          </div>
        )}
