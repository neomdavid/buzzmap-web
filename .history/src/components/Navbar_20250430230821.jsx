...
import { List, X } from "phosphor-react"; // Importing List for hamburger menu
...
const Navbar = () => {
        ...
        >
          <NavLink
            ...
            className={({ isActive }) =>
              ...
              } font-medium text-lg transition-colors duration-300 relative group no-underline`
            }
          >
            {link.name}
          </NavLink>
  ...
  const renderProfile = (darkMode = false) => (
    ...
    >
      ...
      <motion.ul
        tabIndex="0"
        className="dropdown-content mt-2.5 bg-primary text-white rounded-xl z-[10000] w-120 shadow-lg"
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        exit={{ opacity: 0, y: -10 }}
        transition={{ duration: 0.2 }}
        style={{
          maxHeight: "calc(100vh - 100px)", // Prevent overflow on small screens
          overflowY: "auto", // Enable scrolling if content exceeds screen size
        }}
      >
