...
import { Menu, X } from "lucide-react"; // Importing Menu for hamburger icon
...
const Navbar = () => {
  ...
  const toggleDrawer = () => {
    setIsOpen(!isOpen);
    document.body.style.overflow = isOpen ? "auto" : "hidden"; // Set overflow when opening/closing the menu
  };
  ...
  const renderProfile = (darkMode = false, isMobile = false) => (
    <div
      className={`dropdown dropdown-end z-[10000] ${isMobile ? "w-full" : ""}`}
    >
      <div
        tabIndex="0"
        role="button"
