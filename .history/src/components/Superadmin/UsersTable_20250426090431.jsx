...
const StatusCell = ({ value }) => {
  let colorClass = "";
  if (value === "active") colorClass = "text-green-600";
  else if (value === "unverified") colorClass = "text-yellow-500";
  else if (value === "banned") colorClass = "text-red-600";
  else if (value === "removed") colorClass = "text-gray-500";
...
function UsersTable() {
  ...
  useEffect(() => {
    const actualLength = rowData.length;
    const defaultSizes = [5, 10, 20];
    const validSizes = defaultSizes.filter((size) => size <= actualLength);
    setPageSize(validSizes.length > 0 ? validSizes[0] : actualLength || 5);
  }, [rowData]);
  ...
  return (
    ...
    >
      <AgGridReact
        ...
        paginationPageSizeSelector={[pageSize]} // only allow 1 option
