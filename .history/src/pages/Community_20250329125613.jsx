...
const Community = () => {
  ...
  return (
    <main className="pl-6 flex gap-x-6 max-w-[1250px] m-auto relative">
      ...
      {/* Aside Container */}
      <aside
        className={`bg-base-300 px-6 z-100 py-8 shadow-2xl rounded-sm overflow-y-scroll transition-transform duration-300 ease-in-out 
    ...
    lg:w-[35vw] lg:max-w-[450px] ${
      ...
      {/* Close Button (Placed Outside the Aside) */}
      {showAside && (
        <button
          onClick={() => setShowAside(false)}
          className="fixed left-[28vw] top-[50%] transform -translate-x-full -translate-y-1/2 lg:hidden 
               bg-primary text-white p-2 py-10 border-1 border-gray-200 shadow-xl rounded-sm hover:cursor-pointer 
               hover:bg-white hover:text-primary transition-all duration-200 z-50"
        >
