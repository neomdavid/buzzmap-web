...
const SpecificLocation = () => {
  return (
    <main className="text-2xl mt-[-69px] relative">
      {" "}
      ...
      <SideNavDetails />
      <article className="absolute overflow-x-auto z-[100] flex flex-col items-end text-primary right-10 bottom-5 bg-white max-w-[calc(100vw-38vw)]">
        {" "}
        {/* Constrains left expansion */}
        <p className="text-[20px] font-semibold text-left mb-2 w-full">
          {" "}
          {/* Full width title */}
          Most Recent Reports
        </p>
        {/* Scrollable container (starts from right) */}
        <section className="flex flex-row-reverse gap-x-2 text-[13px] overflow-x-auto w-full scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
          <RecentReportCard
          ...
          />
          {/* More cards stack to the LEFT (due to flex-row-reverse) */}
        </section>
      </article>
    </main>
  );
