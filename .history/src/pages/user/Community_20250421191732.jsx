    ...
    <main className="pl-6 text-primary text-lg flex gap-x-6 max-w-[1350px] m-auto relative mt-12">
      <article className="flex-8 shadow-xl p-12 rounded-lg w-[100vw] lg:w-[65vw]">
        ...
        <section className="bg-base-200 px-8 py-5 rounded-lg mb-4">
          ...
          {/* TODO: onclick on this div should show a post card where user can put on information */}
          <div
...
const PostModal = ({ onClose }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
      <div className="bg-white w-[90%] max-w-xl p-6 rounded-xl shadow-lg relative text-primary">
        <button
          onClick={onClose}
        ...
        {/* Replace below with actual PostCard form */}
        <textarea
          className="w-full h-40 border border-primary rounded-md p-3 mb-4"
          placeholder="Share your experience with the community..."
        />

        <button className="bg-primary text-white px-4 py-2 rounded-md hover:bg-accent transition-all">
          Post
        </button>
      </div>
    </div>
