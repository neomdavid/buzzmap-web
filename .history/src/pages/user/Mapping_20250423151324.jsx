...
const Mapping = () => {
  ...
  useEffect(() => {
    ...
    fetch("/quezon_barangays_boundaries.geojson")
      ...
      .then((data) => {
        ...
        const barangaysByDistrict = data.features.reduce((acc, feature) => {
          const districtName = feature.properties.district || "Unknown";
          if (!acc[districtName]) {
            acc[districtName] = [];
          }
          acc[districtName].push(feature);
          return acc;
        }, {});
        setBarangayData(barangaysByDistrict);
      });
  ...
  const handleLocationSelect = (coords) => {
    ...
    if (barangayData) {
      ...
      for (let f of Object.values(barangayData).flat()) {
        let polys = [];
  ...
  const handleDistrictChange = (e) => {
    ...
    setFilteredBarangays(barangayData[selectedDistrict] || []);
  };
  ...
  return (
    <div className="flex flex-col pt-8 px-8 items-center bg-primary text-white h-[95.5vh] mt-[-13px] text-center">
      ...
      >
        ...
        {Object.keys(barangayData || {}).map((districtName, idx) => (
      ...
      <select
        onChange={(e) => {
          const selected = filteredBarangays.find(
      ...
      <div className="w-full z-[10] h-[68vh] rounded-md shadow-md relative">
        ...
        >
          ...
          {filteredBarangays.map((feature, index) => {
