...
const Mapping = () => {
  ...
  const [filterType, setFilterType] = useState("current"); // Added state for filter
  ...
  // Filter function for breeding ground pins
  const filterPins = () => {
    if (filterType === "breedingGrounds") {
      return barangayData?.features.filter(
        (f) => f.properties.riskLevel === "High"
      );
    }
    return barangayData?.features;
  };
    ...
    <div className="flex flex-col pt-8 px-8 items-center bg-primary text-white h-[95.5vh] mt-[-13px] text-center">
      ...
      <div className="flex mb-4 gap-4">
        <button
          ...
          className={`${
            filterType === "current" ? "bg-blue-500" : "bg-gray-500"
          } text-white px-4 py-2 rounded`}
        >
          Current Map
        </button>
        <button
          onClick={() => setFilterType("breedingGrounds")}
          className={`${
            filterType === "breedingGrounds" ? "bg-blue-500" : "bg-gray-500"
          } text-white px-4 py-2 rounded`}
        >
          Breeding Grounds
        </button>
