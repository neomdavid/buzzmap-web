...
const Mapping = () => {
  ...
  const handleLocationSelect = (coords) => {
    // Check if the location is within QC bounds
    if (
      ...
      toast.warn(
        "You are outside Quezon City. The map will be centered to QC."
      );

      // Ensure mapRef is initialized and available
      if (mapRef.current) {
        // Pan the map to QC center (fixed coordinates)
        mapRef.current.panTo({ lat: 14.676, lng: 121.0437 }); // QC center coordinates
        mapRef.current.setZoom(13); // Optional: adjust zoom level if necessary
      }

      // Set the marker position to QC center
      setMarkerPosition({ lat: 14.676, lng: 121.0437 });

      return false; // Exit the function, don't proceed with barangay check
    }

    // Continue with the original barangay data logic if inside QC bounds
    if (barangayData) {
  ...
  return (
    <div className="flex flex-col pt-8 px-8 items-center bg-primary text-white h-[91.8vh] mt-[-13px] text-center">
      ...
      <select
        ...
        className="text-black w-full max-w-md mb-4 px-4 py-2 rounded-md shadow bg-white"
      >
      ...
      <div className="bg-white text-black rounded-md shadow px-4 py-3 w-full max-w-md mb-4 text-left">
      ...
      <div className="w-full z-[10] h-[68vh] rounded-md shadow-md relative">
        ...
        <GoogleMap
          ...
          onLoad={(map) => {
            mapRef.current = map; // Make sure mapRef is set here
          }}
        >
          <Polygon
