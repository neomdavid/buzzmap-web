...
const SingleArticle = () => {
  // State to store the article content and images dynamically fetched from the backend
  ...
  useEffect(() => {
    ...
    // Fetch the article content from the backend (example API call)
    const fetchArticleContent = async () => {
      try {
        const response = await fetch("https://api.example.com/article");
        const data = await response.json();
        setArticleContent(data); // assuming backend sends { title, lastUpdated, content }
      } catch (error) {
  ...
  if (!articleContent) {
    return <div>Loading...</div>; // Show a loading message while content is being fetched
  }
  ...
  return (
    <main className="mt-[-20px] flex flex-col text-primary">
      ...
      <p className="text-center font-bold text-md mt-[-14px]">
        Last updated on{" "}
        {articleContent.lastUpdated || "May 23rd, 2024 at 03:44 PM"}
      </p>
