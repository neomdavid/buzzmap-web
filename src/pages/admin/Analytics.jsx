...
const Analytics = () => {
  ...
  const [initialBarangayNameForMap, setInitialBarangayNameForMap] =
    useState(null);
  ...
  const spikeRecommendationDetails = useMemo(() => {
    ...
    const spikeBarangays = barangaysList.filter(
      (barangay) =>
    ...
    const highestDeathBarangay = spikeBarangays.reduce((highest, current) => {
      const currentDeaths =
        current.status_and_recommendation?.death_priority?.count || 0;
      const highestDeaths =
        highest.status_and_recommendation?.death_priority?.count || 0;
    ...
    return {
      ...
      patternType:
        highestDeathBarangay.status_and_recommendation?.pattern_based?.status ||
        "no_pattern",
      pattern_based:
        highestDeathBarangay.status_and_recommendation?.pattern_based,
      report_based:
        highestDeathBarangay.status_and_recommendation?.report_based,
      death_priority:
        highestDeathBarangay.status_and_recommendation?.death_priority,
  ...
  const selectedNorm = mapSelectedBarangay
    ?.toLowerCase()
    .replace(/[^a-z0-9]/g, "");

  const filteredPosts = Array.isArray(posts?.posts)
    ? posts.posts.filter((post) => {
        const postBarangayNorm = post.barangay
          ?.toLowerCase()
          .replace(/[^a-z0-9]/g, "");
  ...
  const filteredInterventions =
    allInterventionsData?.interventions?.filter((intervention) => {
      const interventionBarangayNorm = intervention.barangay
        ?.toLowerCase()
        .replace(/[^a-z0-9]/g, "");
      ...
      <article className="flex flex-col gap-10">
          ...
          <div className="w-full shadow-sm shadow-lg p-6 py-8 rounded-lg mt-6">
            ...
            >
              ...
              <ul className="space-y-3 mt-2">
                  ...
                  <li className="grid grid-cols-[24px_1fr] gap-2 items-center">
                    ...
                    <div>
                      ...
                      <span className="text-black ml-1">
                        {spikeRecommendationDetails.pattern_based.alert.replace(
                          spikeRecommendationDetails.barangay,
                          spikeRecommendationDetails.barangay
                            .charAt(0)
                            .toUpperCase() +
                ...
                {spikeRecommendationDetails.report_based &&
                  spikeRecommendationDetails.report_based.alert &&
                  spikeRecommendationDetails.report_based.alert.trim() !== "" &&
                  spikeRecommendationDetails.report_based.alert !== "None" && (
                    <li className="grid grid-cols-[24px_1fr] gap-2 items-center">
                      ...
                      <div>
                        ...
                        <span className="text-black ml-1">
                          {spikeRecommendationDetails.report_based.alert.replace(
                            spikeRecommendationDetails.barangay,
                            spikeRecommendationDetails.barangay
                              .charAt(0)
                              .toUpperCase() +
                ...
                {spikeRecommendationDetails.death_priority?.alert && (
                  <li className="grid grid-cols-[24px_1fr] gap-2 items-center">
                    ...
                    <div>
                      ...
                      <span className="text-black ml-1">
                        {spikeRecommendationDetails.death_priority.alert.replace(
                          spikeRecommendationDetails.barangay,
                          spikeRecommendationDetails.barangay
                            .charAt(0)
                            .toUpperCase() +
        ...
        {!isLoadingPatterns &&
          ...
          mapSelectedBarangay && (
            <div className="w-full shadow-sm shadow-lg p-6 py-8 rounded-lg mt-6">
              <p className="text-base-content text-xl font-semibold">
                No spike recommendation available.
              </p>
            </div>
          )}
        <div className="flex flex-col gap-6 gap-y-12 lg:grid lg:grid-cols-12 shadow-sm shadow-lg p-6 py-8 rounded-lg">
          <section className="flex flex-col lg:col-span-7">
            ...
            <div className="mt-[-14px] ml-[-12px] h-full">
              <DengueTrendChart
          ...
          <section className="flex flex-col lg:col-span-5 gap-y-5">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {TABS.map((tab) => (
                <button
                  key={tab.value}
                  className={`px-3 py-1 rounded-full hover:cursor-pointer hover:bg-primary/20 transition-all duration-200 ${
            ...
            {isLoadingBarangays ? (
              ...
              <DengueMap
                ...
                onToggleBreedingSites={() =>
                  setShowBreedingSites(!showBreedingSites)
                }
