...
const InterventionEffectivity = () => {
  ...
  const typeOptions = useMemo(
    () =>
      interventions
        ? Array.from(
            new Set(interventions.map((i) => i.interventionType))
          ).sort()
        : [],
    [interventions]
  );
  ...
  const barangayOptions = useMemo(
    () =>
      interventions
        ? Array.from(new Set(interventions.map((i) => i.barangay))).sort()
        : [],
    [interventions]
  );
    ...
    ? interventions
        .filter((i) => {
          ...
          const matchesBarangay =
            !barangayFilter || i.barangay === barangayFilter;
  ...
  return (
    <main className="flex flex-col w-full ">
      <p className="flex justify-center text-5xl font-extrabold mb-12 text-center md:justify-start md:text-left md:w-[48%]">
        Intervention Effectivity Dashboard
      </p>
      ...
      <div className="max-h-48 overflow-y-auto border rounded bg-white mb-4">
        {isLoading && (
          <div className="p-2 text-gray-500">Loading interventions...</div>
        )}
        {!isLoading && filtered.length === 0 && (
          <div className="p-2 text-gray-500">
            No valid interventions. An intervention must be at least 4 months
            old to analyze effectivity.
          </div>
        )}
        {!isLoading &&
          filtered.map((i) => (
            ...
            >
              <div className="font-semibold text-primary">
                {i.interventionType} - {i.barangay}
              </div>
              <div className="text-xs text-gray-500">
                {new Date(i.date).toLocaleDateString()} | {i.personnel}
              </div>
            </div>
          ))}
      </div>
      {selected && (
        <div className="flex flex-col md:flex-row gap-6 w-full mb-8">
          ...
          <div className="flex-1 min-w-0 w-full bg-neutral-content rounded-lg shadow-sm p-8 flex flex-col items-center gap-6 border border-neutral-200">
            <p className="text-base-content text-3xl font-bold text-left w-full mb-2 tracking-tight">
              Summary Statistics
            </p>
            <div className="flex flex-col md:flex-row gap-6 w-full">
                ...
                <div
                  className={
                    stats.percentChange < 0
                      ? "text-green-600 text-4xl font-extrabold mb-1"
                      : stats.percentChange > 0
                      ? "text-red-600 text-4xl font-extrabold mb-1"
                      : "text-gray-600 text-4xl font-extrabold mb-1"
                  }
                >
                  {stats.totalAfter}
                </div>
              ...
              {/* Percentage Change */}
              <div className="flex-1 min-w-0 w-full flex flex-col items-center bg-white shadow-sm p-8 rounded-2xl transition-transform hover:scale-[1.025] relative group cursor-help">
                <div
                  className={`mb-2 flex items-center justify-center w-14 h-14 rounded-full ${
                ...
                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-4 py-3 bg-gray-900 text-white text-sm rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10 max-w-xs text-center">
                  ...
                  stats.percentChange < 0
                    ? `✅ ${Math.abs(
                        stats.percentChange
                      )}% decrease in dengue cases after intervention. This indicates the intervention was effective in reducing cases.`
                    : typeof stats.percentChange === "number" &&
                      stats.percentChange > 0
        ...
        {selectedId ? (
          ...
          <div className="flex flex-col items-center justify-center h-[350px] bg-white rounded shadow text-gray-400 border">
            ...
            <span className="text-lg">
              Please select an intervention to view effectivity analysis.
            </span>
          </div>
        )}
      </div>
