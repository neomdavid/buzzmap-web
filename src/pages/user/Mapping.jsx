...
const Mapping = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log(
          "Barangay data loaded:",
          barangayGeoJson.features?.length || 0,
          "features"
        );
  ...
  const handleBarangaySelect = (e) => {
    ...
    const matchingBarangay = barangayData.features.find(
      (feature) =>
        normalizeBarangayName(feature.properties.name) ===
        normalizeBarangayName(selectedBarangayName)
    );
    ...
    if (matchingBarangay) {
      ...
      if (lat && lng && !isNaN(lat) && !isNaN(lng)) {
        ...
        let barangayObj = barangaysList?.find(
          (b) =>
            normalizeBarangayName(b.name) ===
            normalizeBarangayName(matchingBarangay.properties.name)
        );
        let patternBased =
          barangayObj?.status_and_recommendation?.pattern_based;
        let patternType = (
          patternBased?.status ||
          matchingBarangay.properties.patternType ||
          "none"
        ).toLowerCase();
        if (!patternType || patternType === "") patternType = "none";
        const patternCardColor =
        ...
        content.innerHTML = `
          <div class=\"bg-white p-4 rounded-lg text-center h-auto\">
            <p class=\"text-4xl font-[900]\" style=\"color:${patternCardColor}\">Barangay ${
          matchingBarangay.properties.displayName ||
          matchingBarangay.properties.name ||
          "Unknown Barangay"
        }</p>
            <div class=\"mt-3 flex flex-col gap-3 text-black\">
              <div class=\"p-3 rounded-lg border-2\" style=\"border-color:${patternCardColor}\">
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-6 left-0 md:left-10 z-10 w-full md:w-auto flex justify-center md:block">
        {showControlPanel && (
          <div className="relative bg-white/60 backdrop-blur-md rounded-lg shadow-xl p-6 w-[70vw] sm:w-[70vw] md:w-[400px] text-primary transition-all duration-300 ease-in-out transform">
            <button
